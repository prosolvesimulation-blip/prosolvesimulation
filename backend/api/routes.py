...
def scan_workspace():
    ...
    try:
        ...
        # Auto-trigger Aster preparation if mesh exists
        if mesh_files:
...
def read_mesh_groups():
    """Reads mesh_groups.json generated by Code_Aster."""
    try:
        data = request.get_json()
        folder_path = data.get('folder_path')
        ...
        if not os.path.exists(json_path):
            return jsonify({"status": "error", "message": "mesh_groups.json not found"}), 404
...
def open_project_config():
    ...
    try:
        ...
        if not os.path.exists(project_file):
            return jsonify({"status": "error", "message": "Project config not found"}), 404
