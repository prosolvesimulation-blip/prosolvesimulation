...
// 🔴 APAGUE ISTO: const API_BASE = 'http://localhost:5000/api';
// 🟢 COLOQUE ISTO:
...
function StructuralWorkflow({ onProjectChange, externalPath, globalSimStatus }) {
    ...
    // 🤝 HANDSHAKE: O Frontend se apresenta ao Backend ao nascer
    useEffect(() => {
        const sayHello = async () => {
            const myPort = window.location.origin;
            console.log(`🚩 [FRONT] Minha porta dinâmica é: ${myPort}`);
            try {
                // "API_BASE" é 'http://localhost:5000/api' (definido no topo do arquivo)
                await fetch(`${API_BASE}/handshake`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ origin: myPort })
                });
                console.log("🤝 [FRONT] Handshake enviado com sucesso!");
            } catch (e) {
                console.error("❌ [FRONT] Erro no Handshake. O main.py está rodando?");
            }
        };
        sayHello();
    }, []);
    ...
    // 🤝 1. PROTOCOLO DE HANDSHAKE (Ao iniciar o componente)
    useEffect(() => {
        const establishConnection = async () => {
            // Descobre a própria porta dinâmica onde o React está rodando
            const myOrigin = window.location.origin; // Ex: http://localhost:56258
            console.log(`🚩 Frontend rodando em porta dinâmica: ${myOrigin}`);

            try {
                // Avisa o Backend (que está fixo na 5000) quem somos nós
                await fetch(`${API_BASE}/handshake`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ origin: myOrigin })
                });
                console.log("🤝 Handshake com Backend: SUCESSO");
            } catch (e) {
                console.error("❌ Falha no Handshake (Backend offline?)");
            }
        };
        establishConnection();
    }, []);
    ...
    const scanFolderForFiles = async (folderPath) => {
        ...
        try {
            ...
            // 👇👇👇 INÍCIO DA CIRURGIA: SALVAR PORTA DO FRONTEND NO PROJETO 👇👇👇
            
            try {
                // 1. Captura quem eu sou (Porta dinâmica atual)
                const frontendInfo = {
                    origin: window.location.origin, // Ex: http://localhost:56789
                    port: window.location.port,     // Ex: 56789
                    timestamp: new Date().toISOString()
                };

                console.log("💾 [METODOLOGIA] Gravando frontend_config.json na pasta do projeto...", frontendInfo);

                // 2. Manda o Backend salvar isso dentro de ./simulation_files
                await fetch(`${API_BASE}/save_frontend_config`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        folder_path: folderPath, // Onde salvar
                        config_data: frontendInfo // O que salvar
                    })
                });
                
                console.log("✅ [METODOLOGIA] Arquivo de porta salvo com sucesso!");

            } catch (saveError) {
                console.error("❌ [METODOLOGIA] Falha ao salvar frontend_config:", saveError);
            ...
            // 👆👆👆 FIM DA CIRURGIA 👆👆👆
                    ...
                    window.projectState.meshAnalysis = {};// Limpa análise anterior
